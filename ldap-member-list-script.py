    ...
    def get_group_members(self, group):
        ...
        if active_directory:
            ...
            for user_dn, user_attrs in user_results:
                if 'cn' in user_attrs:
                    cn_value = user_attrs['cn'][0]
                    if isinstance(cn_value, bytes):
                        cn_value = cn_value.decode(LDAP_ENCODING)
                    dn_to_username[user_dn] = cn_value.lower() if lowercase else username
...
def ldap_zabbix_sync(config):
    ...
    print(">>> Checking memberships for each configured group...")
    for group_name in sorted(config.ldap_groups):
        print(f"\n---------------- Group: {group_name} ------------------")

        if group_name not in ldap_members_list:
            print(f"    LDAP group '{group_name}' not found or error fetching members. Skipping for this group.")
        ...
        sorted_ids = sorted(ldap_members, key=lambda x: x.rsplit('-', 1)[0].strip().lower())
        max_width = max(len(id) for id in sorted_ids)

        # Print with 4 per line, aligned
        for i in range(0, len(sorted_ids), 4):
            line = sorted_ids[i:i+4]
